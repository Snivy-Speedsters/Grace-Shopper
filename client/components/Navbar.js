...
const Navbar = () => {
  ...
  const pages = (loggedIn, admin) => {
    const output = []
    if(loggedIn){
      output.push({name:'All Buddies', path:'/products'},{name:'View Cart', path:'/cart'})
      if(admin){
        output.push({name:'Admin',path:'/admin'})
      }
      return output
    } else {
      return [{name:'Log In', path:'/login'}, {name:'All Buddies', path:'/products'}, {name:'View Cart', path:'/cart'}]
    }
  }

  const user = (loggedIn) => {
    const settings = [{name: 'Profile', path: '/profile'}, {name: 'Log Out', path:'/logout'}]
        ...
        <>
          ...
          <Menu sx={{ mt: '45px' }} id="menu-appbar" anchorEl={anchorElUser}
            ...
            {settings.map((setting) => (
              <MenuItem key={setting.name} onClick={() => {handleRoute(setting.path)}}>
      ...
      <Container maxWidth='xl'>
        <Toolbar>
            ...
            <Menu id="menu-appbar" anchorEl={anchorElNav}
              ...
              {pages(isLoggedIn,isAdmin).map((page) => (
                <MenuItem key={page.name}>
                  <Typography textAlign="center" onClick={() => {handleRoute(page.path)}}>{page.name}</Typography>
                </MenuItem>
              ))}
            </Menu>
          </Box>
          ...
          <Box sx={{ flexGrow: 1, display: { xs: 'none', md: 'flex' } }}>
            {pages(isLoggedIn, isAdmin).map((page) => (
              <Button key={page.name} variant={selected(page.path, currentPage)} onClick={() => {handleRoute(page.path)}} sx={{ my: 2, color: 'white', display: 'block' }}>
                {page.name}
              </Button>
            ))}
          </Box>
